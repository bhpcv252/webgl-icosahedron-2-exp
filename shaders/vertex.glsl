...
void main() {
    ...
    float noise = snoise(4.*vec3(position.x, position.y, position.z + uTime*0.1));

    //noise = smoothstep(0.7, 0.95, noise);

    noise = noise*(2. - noise);

    vec3 modPosition = position + 0.15*noise*normalize(position);

    vModPosition = modPosition;
